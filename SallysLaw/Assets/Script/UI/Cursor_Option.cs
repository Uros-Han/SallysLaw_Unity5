...
public class Cursor_Option : MonoBehaviour {
	...
	void ChgOption(bool bRight)
	{
		...
		case CURSOR_OPTION_STATE.LANGUAGE:
			...
			transform.parent.Find ("Lang").GetChild (1).GetChild (1).GetComponent<UILabel> ().text = Localization.knownLanguages[option.m_iTmpLang];
	...
	void Move_Cursor(bool bDown)
	{
		...
		case CURSOR_OPTION_STATE.SOUND:
			if(bDown){
				transform.localPosition = new Vector2(-470, 205);
		...
		case CURSOR_OPTION_STATE.MUSIC:
			if(bDown){
				transform.localPosition = new Vector2(-470, 105);
				m_CursorState = CURSOR_OPTION_STATE.RESOLUTION;
			}else{
				transform.localPosition = new Vector2(-470, 305);
		...
		case CURSOR_OPTION_STATE.RESOLUTION:
			if(bDown){
				transform.localPosition = new Vector2(-470, 5);
		...
		case CURSOR_OPTION_STATE.SCREEN_MODE:
			if(bDown){
				transform.localPosition = new Vector2(-470, -95);
		...
		case CURSOR_OPTION_STATE.LANGUAGE:
			if(bDown){
				transform.localPosition = new Vector2(-470, -195);
		...
		case CURSOR_OPTION_STATE.CONTROL:
			if(bDown){
				transform.localPosition = new Vector2(-470, -295);
